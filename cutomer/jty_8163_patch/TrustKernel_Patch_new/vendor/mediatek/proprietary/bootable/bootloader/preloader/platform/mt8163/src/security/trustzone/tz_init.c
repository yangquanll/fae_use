...
u32 tee_get_load_addr(u32 maddr)
{
    ...
    DBG_MSG("%s tee_secmem_start  : 0x%x\n\n\n", MOD, tee_secmem_start);
#else
...
#if CFG_TEE_SUPPORT
#if CFG_TRUSTKERNEL_TEE_SUPPORT
    DBG_MSG("%s in tee_sec_config tee_entry_addr = 0x%x\n\n",MOD,tee_entry_addr);
    tz_sec_mem_init(tee_entry_addr,
        tee_entry_addr + tee_secmem_size - 1 - TKCORE_SHM_SIZE_LIMIT,
...
void trustzone_post_init(void)
{
    ...
    DBG_MSG("%s in trustzone_post_init print tee_entry_addr = 0x%x\n",MOD,tee_entry_addr);
	u8 rpmb_key[32] = {0};
	...
	DBG_MSG("%s TEE_BOOT_ARG_ADDR = 0x%x\n", MOD, teearg->tee_boot_arg_addr);
