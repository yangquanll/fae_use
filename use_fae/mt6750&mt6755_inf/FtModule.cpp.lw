...
{
#if MTKCAM_CCAP_USE_META_MODE // For MT8127
...
void FtModPowerOff::exec(Frame *pFrm)
{
    ...
    else if (comPortType == META_SOCKET)
    {
        if (NORMAL_BOOT == getBootMode())
        {
            ...
            const char *strBroadcast = "am broadcast -a android.provider.Telephony.SECRET_CODE -d android_secret_code://3641122";
            system(strBroadcast);
        }
        else
        {
...
void FT_UtilCheckIfFuncExist(FT_UTILITY_COMMAND_REQ  *req, FT_UTILITY_COMMAND_CNF  *cnf)
{
    ...
    case FT_TARGETLOG_CTRL_REQ_ID:
        if (query_op_code <= FT_MOBILELOG_OP_QUERY_LOG_PATH)
        {
