...
static long tee_internal_ioctl(struct tee_context *ctx, unsigned int cmd, void __user *u_arg)
{
	int ret = -EINVAL;
#ifdef TKCORE_KDBG
	...
	case TEE_OPEN_SESSION_IOC:
		ret = tee_do_create_session(ctx, (struct tee_cmd_io __user *) u_arg);
		break;
	...
	case TEE_GET_FD_FOR_RPC_SHM_IOC:
		ret = tee_do_get_fd_for_rpc_shm(ctx, (struct tee_shm_io __user *) u_arg);
		break;

	case TEE_TUI_NOTIFY_IOC:
		ret = tee_tui_notify((uint32_t) (unsigned long) u_arg);
		break;

	case TEE_TUI_WAITCMD_IOC:
		ret = tee_tui_wait((uint32_t __user *) u_arg);
		break;
	...
	case TEE_INSTALL_TA_RESP_IOC:
		ret = tee_install_sp_ta_response(ctx, u_arg);
		break;
	...
	case TEE_INSTALL_SYSTA_IOC:
		ret = tee_install_sys_ta(ctx->tee, u_arg);
		break;

	default:
		pr_err("tkcoredrv: %s: Unknown command: %u\n", __func__, cmd);
		ret = -ENOSYS;
	}
...
static long tee_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case TEE_OPEN_SESSION_IOC:
			{
                struct tee_cmd_io __user *u_cmd = (struct tee_cmd_io __user *) u_arg;
		...
		case TEE_GET_FD_FOR_RPC_SHM_IOC:
			{
				struct tee_shm_io __user *u_shm = (struct tee_shm_io __user *) u_arg;

				if (unlikely(put_user(0, ((uint32_t __user *) &u_shm->buffer) + 1)))
					return -EFAULT;
			}
			break;
		...
		case TEE_INSTALL_TA_IOC:
			{
				...
				if (unlikely(put_user(0, ((uint32_t __user *) &u_spta->ta_binary) + 1)))
					return -EFAULT;

				if (unlikely(put_user(0, ((uint32_t __user *) &u_spta->response_len) + 1)))
					return -EFAULT;
			}
			break;
		...
		case TEE_INSTALL_SYSTA_IOC:
			{
				struct tee_ta_inst_desc __user *u_inst_desc =
					(struct tee_ta_inst_desc __user *) u_arg;

				if (unlikely(put_user(0, ((uint32_t __user *) &u_inst_desc->uuid) + 1)))
					return -EFAULT;

				if (unlikely(put_user(0, ((uint32_t __user *) &u_inst_desc->ta_buf) + 1)))
					return -EFAULT;

				if (unlikely(put_user(0, ((uint32_t __user *) &u_inst_desc->resp_size) + 1)))
					return -EFAULT;
			}
