...
static inline void save_atf_log_buf_to_log_store(void)
{
    ...
    ((struct sram_log_header *)SRAM_LOG_ADDR)->dram_buf[LOG_PL_LK].atf_log_addr = atf_log_buf_start;
    ((struct sram_log_header *)SRAM_LOG_ADDR)->dram_buf[LOG_PL_LK].atf_log_len = ATF_LOG_BUFFER_SIZE;
}
...
u32 tee_get_load_addr(u32 maddr)
{
    ...
    DBG_MSG("%s align_addr: 0x%llx, limit_addr: 0x%llx\n", MOD, align_addr, limit_addr);
    tee_secmem_start = (u32)mblock_reserve_ext(&bootarg.mblock_info,
        (u64)(tee_secmem_size + ROUNDUP(ATF_LOG_BUFFER_SIZE, ATF_MEM_ALIGNMENT)), (u64)ATF_MEM_ALIGNMENT,
        limit_addr, 0, "tee-reserved");
    DBG_MSG("%s TEE reserved_addr: 0x%x, reserved_size: 0x%x\n", MOD, tee_secmem_start, tee_secmem_size + ROUNDUP(ATF_LOG_BUFFER_SIZE, ATF_MEM_ALIGNMENT));
