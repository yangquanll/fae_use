...
static long tee_session_internal_ioctl(struct tee_session *sess, unsigned int cmd, unsigned long arg)
{
	int ret;
	struct tee *tee;
	...
	case TEE_INVOKE_COMMAND_IOC:
		ret = tee_do_invoke_command(sess, (struct tee_cmd_io __user *)arg);
		break;
