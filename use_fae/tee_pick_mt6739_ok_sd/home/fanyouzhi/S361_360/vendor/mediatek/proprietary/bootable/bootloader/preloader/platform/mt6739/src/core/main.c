...
extern boot_mode_t ab_boot_mode;
#endif
...
#if defined(MTK_AB_OTA_UPDATER)
    struct part_info_t loader_ext_a_info, loader_ext_b_info;

    if (!memcmp(ab_suffix, BOOTCTRL_SUFFIX_A, 2)) {
        memcpy(part_name, "loader_ext_a", 13);
        ret = mt_get_part_info_by_name("loader_ext_a", &loader_ext_a_info);
        if (ret) {
            print("get loader_ext_a_info error\n");
        }
    } else {
        memcpy(part_name, "loader_ext_b", 13);
        ret = mt_get_part_info_by_name("loader_ext_b", &loader_ext_b_info);
        if (ret) {
            print("get loader_ext_b_info error\n");
        }
    }
#else
    struct part_info_t loader_ext1_info, loader_ext2_info;

    ret = mt_get_part_info_by_name("loader_ext1", &loader_ext1_info);
    if (ret) {
        print("get info1 error\n");
    }

    ret = mt_get_part_info_by_name("loader_ext2", &loader_ext2_info);
    if (ret) {
        print("get info2 error\n");
    }

    print("dram, active1: %d, active2: %d\n", loader_ext1_info.active, loader_ext2_info.active);
#endif //MTK_AB_OTA_UPDATER

#ifdef MTK_SECURITY_SW_SUPPORT
#if !defined(MTK_AB_OTA_UPDATER)
    if (loader_ext1_info.active == 0 && loader_ext2_info.active > 0)
        memcpy(part_name, "loader_ext2", 12);
    else
        memcpy(part_name, "loader_ext1", 12);
#endif //MTK_AB_OTA_UPDATER
...
#if defined(MTK_AB_OTA_UPDATER)
    if (!memcmp(ab_suffix, BOOTCTRL_SUFFIX_A, 2)) {
        print("load loader_ext_a_dram\n");
        if (blkdev_read(bdev, loader_ext_a_info.addr, sizeof(part_hdr_t), &hdr, EMMC_PART_USER) != 0) {
        ...
        if (blkdev_read(bdev, loader_ext_a_info.addr + sizeof(part_hdr_t), hdr.info.dsize, (u8 *)CFG_LOADER_EXT_DRAM_ADDR, EMMC_PART_USER) != 0) {
            print("load image error\n");
            ASSERT(0);
        }
    } else {
        print("load loader_ext_b_dram\n");
        if (blkdev_read(bdev, loader_ext_b_info.addr, sizeof(part_hdr_t), &hdr, EMMC_PART_USER) != 0) {
            print("read header error\n");
            ASSERT(0);
        } else {
            if (hdr.info.magic != PART_MAGIC) {
        ...
        if (blkdev_read(bdev, loader_ext_b_info.addr + sizeof(part_hdr_t), hdr.info.dsize, (u8 *)CFG_LOADER_EXT_DRAM_ADDR, EMMC_PART_USER) != 0) {
            print("load image error\n");
            ASSERT(0);
        }
    }
#else
    if (loader_ext1_info.active == 0 && loader_ext2_info.active > 0) {
        print("load loader_ext2_dram\n");
        if (blkdev_read(bdev, loader_ext2_info.addr, sizeof(part_hdr_t), &hdr, EMMC_PART_USER) != 0) {
            print("read header error\n");
            ASSERT(0);
        } else {
            if (hdr.info.magic != PART_MAGIC) {
        ...
        if (blkdev_read(bdev, loader_ext2_info.addr + sizeof(part_hdr_t), hdr.info.dsize, (u8 *)CFG_LOADER_EXT_DRAM_ADDR, EMMC_PART_USER) != 0) {
            print("load image error\n");
            ASSERT(0);
        }
    } else {
        print("load loader_ext1_dram\n");
        if (blkdev_read(bdev, loader_ext1_info.addr, sizeof(part_hdr_t), &hdr, EMMC_PART_USER) != 0) {
            print("read header error\n");
            ASSERT(0);
        } else {
            if (hdr.info.magic != PART_MAGIC) {
        ...
        if (blkdev_read(bdev, loader_ext1_info.addr + sizeof(part_hdr_t), hdr.info.dsize, (u8 *)CFG_LOADER_EXT_DRAM_ADDR, EMMC_PART_USER) != 0) {
            print("load image error\n");
            ASSERT(0);
        }
    }
...
unsigned int bldr_load_loader_ext_etc()
{
    ...
    u32 size = 0;
    unsigned long loader_ext1_active = 0, loader_ext2_active = 0;
    u8 img_name[32] = {0};
    u32 img_size = 0;
    u32 ret = 0;
...
#if defined(MTK_AB_OTA_UPDATER)
    if (!memcmp(ab_suffix, BOOTCTRL_SUFFIX_A, 2)) {
        part_loader_ext = part_get("loader_ext_a");
        if (NULL == part_loader_ext) {
            print("loader_ext_a not found\n");
            return -1;
        } else {
            ret = sec_get_img_size("loader_ext_a", img_name, &img_size);
            if (ret) {
                print("get ext_a size fail: %x", ret);
            }
            /* img_size = image size + signature */
            return part_load(bdev, part_loader_ext, &addr, sizeof(part_hdr_t) + img_size, &size);
        }
    } else {
        part_loader_ext = part_get("loader_ext_b");
        if (NULL == part_loader_ext) {
            print("loader_ext_b not found\n");
            return -1;
        } else {
            ret = sec_get_img_size("loader_ext_b", img_name, &img_size);
            if (ret) {
                print("get ext_b size fail: %x", ret);
            }
            /* img_size = image size + signature */
            return part_load(bdev, part_loader_ext, &addr, sizeof(part_hdr_t) + img_size, &size);
        }
...
#else
    // check the active bit of loader_ext partition
    part_loader_ext = part_get("loader_ext1");
    if (NULL == part_loader_ext) {
        print("loader_ext1 not found\n");
        return -1;
    } else {
        loader_ext1_active = mt_part_get_active_bit(part_loader_ext);
    }

    // check the active bit of loader_ext2 partition
    part_loader_ext = part_get("loader_ext2");
    if (NULL == part_loader_ext) {
        print("loader_ext2 not found\n");
        //return -1; Comment it since we could load loader_ext1 partition to boot
    } else {
        loader_ext2_active = mt_part_get_active_bit(part_loader_ext);
    }

    print("etc, active1: %d, active2: %d\n", loader_ext1_active, loader_ext2_active);

    // load partition
    if (loader_ext1_active == 0 && loader_ext2_active > 0) {
        print("load loader_ext2_etc\n");
        part_loader_ext = part_get("loader_ext2");
        ret = sec_get_img_size("loader_ext2", img_name, &img_size);
        if (ret) {
            print("get ext2 size fail: %x", ret);
        }

        if (part_loader_ext != NULL) {
            /* img_size = image size + signature */
            return part_load(bdev, part_loader_ext, &addr, sizeof(part_hdr_t) + img_size, &size);
        } else {
            return -1;
        ...
        print("load loader_ext1_etc\n");
        part_loader_ext = part_get("loader_ext1");
        ret = sec_get_img_size("loader_ext1", img_name, &img_size);
        if (ret) {
            print("get ext1 size fail: %x", ret);
        }
...
static int bldr_handshake(struct bldr_command_handler *handler)
{
    ...
    case NORMAL_BOOT:
        ...
        /* security check            */
        /* ------------------------- */
#ifndef QIHU_EXTEND_SECURE_BOOT_SUPPORT
        if (TRUE == isSLA) {
