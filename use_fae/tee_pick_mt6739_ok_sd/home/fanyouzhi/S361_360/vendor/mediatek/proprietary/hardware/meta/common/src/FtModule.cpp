...
void FT_UtilCheckIfFuncExist(FT_UTILITY_COMMAND_REQ  *req, FT_UTILITY_COMMAND_CNF  *cnf)
{
    ...
    switch (query_ft_msg_id)
    {
		...
		case FT_UTILITY_COMMAND_REQ_ID:
			...
			else if(query_op_code == FT_UTILCMD_SET_ATM_FLAG)
			{
			    cnf->status = FT_CNF_OK;
				META_LOG("[Meta][FT] FT_UTILCMD_SET_ATM_FLAG = yes");
			}
			else if(query_op_code == FT_UTILCMD_SET_PRODUCT_INFO)
			{
				cnf->status = FT_CNF_OK;
				META_LOG("[Meta][FT] FT_UTILCMD_SET_PRODUCT_INFO = yes");
			}
			break;
